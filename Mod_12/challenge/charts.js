...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var metaSampleID = data.metadata.filter(data => data.id == sample);
    console.log(metaSampleID);

    // Create a variable that holds the first sample in the array.
    // 2. Create a variable that holds the first sample in the metadata array.
    var fstSample = metaSampleID[0];
    console.log(fstSample);

    // Create variables that hold the otu_ids, otu_labels, and sample_values.
    ///var del3otuID = fstSample.otu_ids;
    //var del3otuLabels = fstSample.otu_labels;
    //var del3sampleValues = fstSample.sample_values;

    // 3. Create a variable that holds the washing frequency.
    var washFreq = parseFloat(fstSample.wfreq);
